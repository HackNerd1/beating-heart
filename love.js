...
class Point {
   ...
   */
  length(length) {
    if (typeof length == "undefined") return Math.sqrt(this.x * this.x + this.y * this.y); // 面积
    this.normalize();
    this.x *= length;
    this.y *= length;
    return this;
...
class Particle {
   ...
   */
  update(deltaTime) {
    this.position.x += this.velocity.x * deltaTime; // 通过速度和时间计算位移
    this.position.y += this.velocity.y * deltaTime;
    this.velocity.x += this.acceleration.x * deltaTime; // 通过加速度计算速度
    this.velocity.y += this.acceleration.y * deltaTime;
    this.age += deltaTime; // 生命周期
  }
...
const main = (canvas) => {
  ...
  function pointOnHeart(t, callback) {
    // 完美心形函数
  ...
  function render() {
    ...
    for (var i = 0; i < amount; i++) {
      ...
      const getThrottleHeartPoint = throttle(pointOnHeart, 10000);
      getThrottleHeartPoint(t, (point) => {
        var dir = point.clone().length(settings.particles.velocity);
        // console.log();
        // console.log(point.x);
        particles.add(canvas.width / 2 + point.x, canvas.height / 2 - point.y, dir.x, -dir.y);
      });
    }
...
const throttle = (func, wait) => {
  ...
  return function () {
    // arguments[0] > 3 ||
    if (arguments[0] < 0.2 && arguments[0] > -0.2) {
      // console.log(t);
    ...
    } else if (arguments[0] > 3 || arguments[0] < -3) {
      // return func(...arguments);
...
initAnimation();
main(document.getElementById("pinkboard"));
